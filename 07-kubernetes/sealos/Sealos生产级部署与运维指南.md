...
# 2. 配置命名空间隔离
cat >> network-policy-allow-same-namespace.yaml << 'EOF'
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
...
kubectl apply -f network-policy-allow-same-namespace.yaml

# 3. 启用 Pod 安全策略（Pod Security Policy）
...
# 3. 限制节点直接访问
kubectl adm cordon <node-name>  # 标记为不可调度
kubectl taint nodes <node-name> NoSchedule:NoExecute
```
...
- **Sealos 官方网站**：https://sealos.io
- **Sealos GitHub 仓库**：https://github.com/labring/sealos
...
- **云原生技术路线图**：https://github.com/cncf/roadmaps
...
- **最后更新**：2026-03-09
- **维护者**：devinyan
- **反馈渠道**：如有问题或建议，请提交 Issue 或 Pull Request
...
本文档中的所有配置示例、脚本和命令仅供参考，请在生产环境使用前充分测试并根据实际环境调整。作者不对使用本文档造成的任何损失承担责任。
