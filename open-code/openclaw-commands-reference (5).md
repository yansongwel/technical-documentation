...
# OpenClaw 性能与记忆深度优化指南 (基于社区最佳实践)

> **参考来源**：
> 1. [Researcher_王十三 - 核心文件深度配置指南](https://x.com/researchwang/status/2030151219998793775)
> 2. [WY - 长期记忆解决方案推荐](https://x.com/akokoi1/status/2030214411340664986)

## 一、给 OpenClaw 换个“大脑”（核心文件优化）

不要以为插件越多 AI 越聪明，真正决定 OpenClaw 智商上限的是底层的 `.md` 配置文件。

### 1. SOUL.md：决定是平庸还是卓越
这是最基础的文件，定义了代理的性格、核心价值观和长期指令。
* **核心身份与人格**：明确角色设定和沟通风格。例如：“简单问题一针见血，复杂系统详细拆解”、“所有关键结论必须加粗”。
* **核心价值观与绝对红线**：明确什么不能做。例如：“绝对禁止泄露隐私”、“执行删除操作前必须强制挂起请求确认”。
* **长期指令与生存法则**：例如：“在响应指令前，必须先静默读取并更新全局记忆文件”。
> **秘诀**：SOUL.md 应该保持简短和具体。模糊的指令（“要有帮助”）产生模糊的行为，具体的指令（“最多5个要点”）产生特定的行为。

### 2. AGENTS.md：工作指南
详细记录任务处理流程、决策逻辑。
* **唤醒协议**：每次会话前静默读取 SOUL.md、USER.md、短期记忆、核心大纲。
* **记忆库新陈代谢**：每日流水追加、精华提炼到全局 MEMORY.md、定期向量同步。
* **护主与绝对红线**：明确行动界限，破坏性操作拦截。
* **行动域界限**：本地随意操作，越界（发推、发邮件、调用外部API）必报。

### 3. USER.md：写给 OpenClaw 的“使用说明书”
* **基础参数**：包含正确的时区（影响定时任务）。
* **沟通与排版癖好**：最讨厌看到什么（如废话连篇），最喜欢看到什么格式（如短句、结论前置）。
* **当前焦点**：最近在肝什么，避免 AI 给出不相干的建议。
* **隐秘的细节与雷区**：细微的偏好设定。

### 4. HEARTBEAT.md：心跳机制（主动任务）
让代理在后台定期检查任务，在没有提示的情况下主动提供价值。
* **定时抓取**：定时检查特定数据（股市、推文点赞、CI/CD日志）并提醒。
* **每日简报**：例如早上 07:30 推送昨夜核心数据和今日待办。
* **条件触发**：例如当 $BTC 波动超过 3% 时立刻最高级别提醒。

### 5. TOOLS.md vs SKILLS
* **Tools 是器官**：决定 OpenClaw 能否做某事（手）。
* **Skills 是教科书**：教 OpenClaw 如何组合工具（说明书）。
> 注意：高版本需在 openclaw.json 中将 tools 权限设为 full。

### 6. IDENTITY.md：对外身份
决定对外的展现：姓名、物种、氛围（极简、幽默等）。与 SOUL.md 分离设计，可以随时“换衣服”而不改变“性格”。

### 7. BOOTSTRAP.md：初始化拷打
用于首次启动，快速对齐身份和偏好，并将信息注入底层配置文件。

---

## 二、长期记忆解决方案 (Total Recall 实践)

解决 OpenClaw 经常忘记几天前事情的痛点。除了内置文件系统和 LanceDB (memory-lancedb-pro)，强烈推荐 **Total Recall** 记忆系统。

### Total Recall 自主记忆系统
**核心优势**：自主运行，无需手动干预，自动记录观察、总结并整理长期记忆。

**五层架构**：
1. **Layer 1: Observer (观察器)**
   * 定时 (Cron，15-30分钟) 扫描最近会话 JSONL。
   * 自动压缩最近消息，打上优先级，追加到 `observations.md`。
2. **Layer 2: Reflector (反思器)**
   * 当观察记录超过 8000 字时触发。
   * 整合相关条目，丢弃过时低优先级信息，实现 40-60% 瘦身。
3. **Layer 3: Session Recovery (会话恢复)**
   * 每次 `/new` 或 `/reset` 时运行，捕捉遗漏的会话。
4. **Layer 4: Reactive Watcher (反应式监控)**
   * (Linux inotify) 会话文件高频写入时主动触发观察器，突破 Cron 间隔限制。
5. **Layer 5: Pre-compaction hook (预压缩钩子)**
   * 紧急捕获 OpenClaw 即将压缩的上下文。

**v2.0 增强 (Ambient Intelligence Engine - AIE)**：
引入环境智能引擎，可以监控外部系统（日历、Gmail、Fitbit、文件系统等），在后台进行“思考 (rumination)”，并将紧急事项主动推送到前台。

**如何接入 Total Recall**：
1. 安装：`clawdhub install total-recall`
2. 配置 API Key：`OPENROUTER_API_KEY=xxx`
3. 运行设定：`bash skills/total-recall/scripts/setup.sh`
4. 引导代理：在 `MEMORY.md` 中加入：`At session startup, read memory/observations.md for cross-session context.`

---

## 优化小结
1. **完善配置**：根据本指南逐一修改 `SOUL.md`、`AGENTS.md`、`USER.md` 和 `HEARTBEAT.md`。
2. **主动出击**：写好心跳配置，让代理成为监控助手而不仅是问答机器人。
3. **部署记忆**：使用 Total Recall 等系统，确保上下文永久保存并自动精简。
