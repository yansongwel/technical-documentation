...
本步骤的目标是：让每个 MySQL 节点满足 InnoDB Cluster 的“可加入条件”（GTID、binlog、组复制通信参数等），并把“每台机器必须不同”的参数显式固定下来，避免后续加入集群时报错或出现节点互相不可达。

**必须修改项清单（每台节点都要按实际填写）**

| 配置项 | 作用 | 示例 | 常见错误 |
|--------|------|------|----------|
| `datadir` / `socket` / `pid-file` | 数据目录与套接字路径 | `/data/mysql` | 数据盘未挂载/权限错误导致 mysqld 启动失败 |
| `server_id` | 复制身份（必须唯一） | 101/102/103 | 重复会导致复制/恢复失败 |
| `loose-group_replication_local_address` | 组复制通信地址 | `IP:33061` | 写成不可达 IP/未放通 33061 |
| `loose-group_replication_group_seeds` | 种子列表（全员） | 3 节点列表 | 只写自己或写错 IP/端口 |
| `loose-group_replication_ip_allowlist` | 允许加入的网段 | `192.168.1.0/24` | 过小导致成员互相握手失败 |
| `report_host` | 成员对外“公告地址” | 节点 IP | 写成 127.0.0.1/主机名不可解析 |

**3 节点填写示例（把差异点集中列出来，避免漏改）**

| 节点 | `server_id` | `report_host` | `loose-group_replication_local_address` |
|------|-------------|---------------|-----------------------------------------|
| MySQL-01 | 101 | 192.168.1.101 | 192.168.1.101:33061 |
| MySQL-02 | 102 | 192.168.1.102 | 192.168.1.102:33061 |
| MySQL-03 | 103 | 192.168.1.103 | 192.168.1.103:33061 |
...
# 数据目录规划：建议使用独立数据盘挂载点，避免系统盘 I/O 干扰
datadir = /data/mysql            # ★ ← 根据实际挂载点修改
# Socket 用于本机管理连接（systemd、备份脚本等通常会用到）
...
# ====================
# X Protocol（可选，但建议显式固定）
# ====================

# 33060 用于 MySQL X Protocol（mysqlsh/Router 某些场景会用到）
mysqlx_port = 33060
...
# 专用数据库主机建议开启：MySQL 会按内存自动设置 buffer pool 等参数（更不易配错）
innodb_dedicated_server = ON     # ⚠️ 如果与其他大服务混部，建议改为 OFF 并手工设 buffer pool
# 让 InnoDB 绕过文件系统缓存，减少双缓存带来的抖动（常见生产推荐）
innodb_flush_method = O_DIRECT
# 事务提交刷盘策略：1 = 最强一致性（更安全，写入延迟略高）
innodb_flush_log_at_trx_commit = 1
# binlog 刷盘策略：1 = 每次提交刷盘（更安全，写入延迟略高）
sync_binlog = 1
...
# 每个节点必须唯一（建议按节点编号规划，如 101/102/103）
server_id = 101                  # ★ ← 每个节点唯一（示例：101/102/103）
# 打开 binlog：组复制/分布式恢复依赖 binlog/GTID
log_bin = mysql-bin
# ROW 是组复制推荐格式（最利于一致性与冲突检测）
binlog_format = ROW              # ⚠️ 8.4 会提示弃用；按官方演进评估替换
# FULL 便于复制与恢复（记录更完整的行镜像）
binlog_row_image = FULL
# binlog 保留时长（秒）：至少覆盖“备份 + 回放窗口”（示例 7 天）
binlog_expire_logs_seconds = 604800

# 开启 GTID：InnoDB Cluster 的基础要求（用于分布式恢复与一致性判断）
gtid_mode = ON
enforce_gtid_consistency = ON
# 从库执行的事务也写入 binlog（组复制/恢复链路需要）
log_replica_updates = ON
# 关闭 binlog checksum：减少链路复杂性（组复制常用基线）
...
本步骤的目标是：用 MySQL Shell（`mysqlsh`）把 3 台 MySQL 实例“改造成可加入 InnoDB Cluster 的实例”，并完成集群元数据初始化、成员加入与状态校验。

你会执行 3 类命令：

| 命令 | 在做什么 | 你应该看到什么变化 |
|------|----------|--------------------|
| `dba.configureInstance()` | 对单个 MySQL 实例做“入群前置配置”，并创建集群管理账号 | 实例参数满足集群要求；出现 `The instance ... is valid for InnoDB Cluster usage.` |
| `dba.createCluster()` | 创建集群元数据并引导组复制（选举出 Primary） | 集群对象可被 `dba.getCluster()` 获取；状态里出现 `PRIMARY` |
| `cluster.addInstance()` | 把其它实例加入集群（分布式恢复：增量或克隆） | 新成员进入 `ONLINE`，角色为 `SECONDARY` |
...
`dba.configureInstance()`：对每台 MySQL 做“入群准备”，并创建集群管理员 `icadmin`（后续所有集群管理命令都建议用该账号执行，而不是 root）。
...
`dba.createCluster()`：在 MySQL-01 上创建集群元数据并引导组复制。该命令会在内部创建 InnoDB Cluster 的 metadata，并让当前实例成为第一个成员（随后加入其它成员）。
...
`cluster.addInstance()`：把 MySQL-02/03 加入集群。

- `recoveryMethod:'incremental'` 表示优先走“增量分布式恢复”（基于 GTID/复制拉取缺失事务）
- 当提示有“errant GTIDs”（存在不属于集群的事务）时，增量恢复会被拒绝，此时应改用 `clone` 或先清理 GTID（必须在测试环境演练后再用于生产）
...
本步骤的目标是：在 Router 节点生成一份“可随集群拓扑自动更新”的 Router 配置（metadata-cache 模式）。之后业务侧只需要连接 Router（或 VIP/LB），不需要感知哪个节点是 Primary。
...
`mysqlrouter --bootstrap`：从 InnoDB Cluster 读取元数据，生成 `mysqlrouter.conf`，并在 MySQL 内创建 Router 用的账号/权限（因此需要使用 `icadmin` 连接）。
...
执行完成后，Router 配置目录里会出现类似 RW/RO 端口（常见为 6446/6447）。RW 端口始终指向当前 Primary；RO 端口会把连接分配到 Secondary。
...
Router-02（BACKUP）：与 Router-01 **只有少量差异**，其余必须保持一致（尤其是 `virtual_router_id`、`auth_pass`、VIP、网卡）。
...
vrrp_instance VI_1 {
  ...
  #   - 虚拟机：常见为 ens33/eth0，需以 `ip addr` 结果为准
  #   - 云主机：通常使用内网网卡与内网 IP
  interface eth0                         # ★ ← 必须与 Router-01 在同一二层网段
  virtual_router_id 51                   # ★ ← 必须与 Router-01 相同
  priority 110                           # ★ ← 必须低于 Router-01（例如 120/110）
  advert_int 1
  authentication {
    auth_type PASS
    auth_pass MySQLRouterVIP2026         # ★ ← 必须与 Router-01 相同
  }
  virtual_ipaddress {
    192.168.1.200/24                     # ★ ← 必须与 Router-01 相同
  }
  track_script {
    chk_mysqlrouter
  }
...
```bash
# 在当前持有 VIP 的 Router 节点执行（通常是 Router-01）
systemctl stop mysqlrouter-prod.service
sleep 3

# 在 Router-02 执行，确认 VIP 已漂移
ip a | grep 192.168.1.200
```

> ⚠️ 重要：部分云环境/三层网络不支持 Keepalived 漂移 VIP，需要使用云厂商提供的 LB/VIP 能力。
