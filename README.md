...
| 01 | [数据库](#01--数据库) | MySQL · Redis · MongoDB · Elasticsearch · PostgreSQL · TiDB · ClickHouse · Cassandra · InfluxDB |
| 02 | [消息队列 & 流处理](#02--消息队列--流处理) | Kafka · RabbitMQ · RocketMQ · Pulsar · NATS |
| 03 | [CI/CD 持续集成与交付](#03--cicd-持续集成与交付) | Jenkins · Jenkins X · ArgoCD · GitLab CI · Tekton · Drone · Devtron · KubeVela · OpenKruise |
| 04 | [API 网关 & Ingress](#04--api-网关--ingress) | Nginx · OpenResty · APISIX · Higress · Traefik · Kong · Envoy |
| 05 | [服务网格](#05--服务网格-service-mesh) | Istio · Linkerd · Consul Connect |
...
| 07 | [Kubernetes](#07--kubernetes-集群) | 集群部署（kubeadm · kubeasz · kubespray · KuboardSpray · kind · Sealos · Rancher/RKE2/K3s） · 存储 · 网络 · 安全 · Helm · Kustomize · Velero |
...
| 12 | [负载均衡 & 高可用](#12--负载均衡--高可用) | HAProxy · Keepalived · LVS |
| 13 | [任务调度](#13--任务调度) | Airflow · XXL-Job · Elastic-Job |
| 14 | [网络 & VPN](#14--网络--vpn) | WireGuard · OpenVPN · Calico · Cilium |
| 15 | [IaC & 配置管理](#15--iac--配置管理) | Ansible · SaltStack · Terraform · Vagrant |
| 16 | [语言运行环境](#16--语言运行环境) | Python · Go · PHP · Java · Node.js |
| 17 | [AI 编程工具](#17--ai-编程工具) | Claude Code · OpenCode · Kubernetes Operator |
...
- **生产级别**：配置均来源于真实服务器验证（Docker/Docker Compose 伪集群 + 本地服务器安装）
- **多部署方式**：提供 Linux 原生服务（主线）与 Docker 快速体验两种方式
- **详细注释**：配置文件提供完整中文逐行注释
- **架构图示**：每个组件包含 Mermaid 架构图与部署拓扑
- **全面覆盖**：覆盖 SRE 涉及的 80+ 主流开源中间件
- **验证驱动**：所有文档内容（下载地址、配置参数、管理命令）均经过实际验证后输出

---

## 文档规范

每个中间件部署文档统一遵循以下 11 章结构（数据库类包含第 8 章，其他服务跳过）：

```
01-简介（特性·场景·架构图·版本说明）
02-版本选择指南
03-生产环境规划（高可用架构图·节点角色·网络端口）
04-生产环境部署（前置准备·部署步骤·集群初始化·安装验证）
05-关键参数配置说明
06-快速体验部署（开发/测试环境）
07-日常运维操作（管理命令·备份恢复·扩缩容·版本升级）
08-使用手册（数据库专项：连接认证·CRUD·用户权限·慢查询分析）
09-监控与告警接入（Exporter·关键指标·Grafana Dashboard·告警规则）
10-注意事项与生产检查清单
11-参考资料
```

> 当某中间件存在多种生产级集群方案或主流版本差异较大时，拆分为独立文档并额外提供选型指南。
...
> **注意**：若某个命令或配置仅适用于其中一个系统，必须用 `# ── Rocky Linux 9 ──` 或 `# ── Ubuntu 22.04 ──` 注释块单独标注，禁止混写造成歧义。
...
│
├── 00-linux-os/                  # Linux 操作系统基础
│   ├── rocky-linux/              # Rocky Linux 系统配置
│   ├── ubuntu/                   # Ubuntu 系统配置
...
│   ├── mysql/                    # 关系型数据库
│   ├── redis/                    # 内存数据库/缓存
│   │   ├── redis-cluster-production/   # Redis Cluster 生产部署
│   │   └── compose-sentinel/           # Redis Sentinel 快速体验
│   ├── mongodb/                  # 文档数据库
│   ├── postgresql/               # 高级关系型数据库
│   ├── tidb/                     # 分布式 HTAP 数据库
│   ├── clickhouse/               # 列式分析数据库
...
│   ├── kind/                     # Kind 本地开发测试集群
│   ├── sealos/                   # Sealos 集群生命周期管理
│   ├── rancher/                  # Rancher Manager / RKE2 / K3s
│   ├── storage/                  # 持久化存储方案
│   ├── networking/               # 网络插件与策略
│   ├── security/                 # 安全加固与 RBAC
...
| 模块 | 简介 | 文档 |
|------|------|------|
| **Rocky Linux** | 源配置、最小化安装、安全基线 | [文档](./00-linux-os/rocky-linux/) |
| **Ubuntu** | 源配置（阿里云/清华源）、初始化规范 | [文档](./00-linux-os/ubuntu/) |
| **内核参数优化** | sysctl 生产调优（网络/内存/文件句柄） | [文档](./00-linux-os/kernel-optimization/) |
| **iptables** | 四表五链详解、规则编写、生产示例 | [文档](./00-linux-os/iptables/) |
| **firewalld** | zone 管理、rich rules、生产配置 | [文档](./00-linux-os/firewalld/) |
| **系统初始化** | 主机名/时区/NTP/SSH 加固/用户规范 | [文档](./00-linux-os/system-init/) |
...
| **OpenKruise** | 云原生应用自动化管理引擎 | [文档](./03-cicd/openkruise/) |
...
| **Sealos** | 集群生命周期管理，一条命令交付 K8s | [文档](./07-kubernetes/sealos/) |
| **Rancher / RKE2 / K3s** | 企业级多集群管理与轻量发行版 | [文档](./07-kubernetes/rancher/) |
...
| **Vagrant** | 本地多节点 VM 快速验证环境 | [指南](./15-iac/vagrant/Vagrant使用指南.md) |

---

### 本地验证环境（Vagrant）

> 本文档库的文档均将在服务器 / 本地 VM 中验证后再交付，确保零错误。以下 Vagrantfile 均基于 **Rocky Linux 9**。
...
| [Vagrantfile_3Nodes_DB](./15-iac/vagrant/启动示例文件/Vagrantfile_3Nodes_DB) | 数据库集群（MySQL/Redis/MongoDB/ES/Kafka） | 3 | 3x16 = 48 GB | 192.168.34.101-103 |
| [Vagrantfile_5Nodes_K8s](./15-iac/vagrant/启动示例文件/Vagrantfile_5Nodes_K8s) | K8s 集群（Sealos/kubeadm/kubeasz） | 5（3M+2W） | 5x16 = 80 GB | 192.168.35.101/111 |
| [Vagrantfile_7Nodes_FullHA](./15-iac/vagrant/启动示例文件/Vagrantfile_7Nodes_FullHA) | 全量 HA（K8s+Ceph存储） | 7（3M+3W+1S） | 7x16 = 112 GB | 192.168.36.101/111/121 |
...
> 详细用法参考：[Vagrant 使用指南](./15-iac/vagrant/Vagrant使用指南.md)
...
| 工具 | 简介 | 文档 |
|------|------|------|
| **Claude Code** | Anthropic AI 编程助手命令详解 | [文档](./17-AI/claude-code-命令详解.md) |
| **OpenCode** | 开源 AI 编程终端工具 | [文档](./17-AI/OpenCode%20完整使用指南.md) |
| **K8s Operator 开发** | Kubernetes Operator 开发指南 | [文档](./17-AI/KUBERNETES_OPERATOR_GUIDE.md) |
...
2. **命名规范**：中间件部署文档使用中文描述性命名（如 `Redis-Cluster生产级部署与运维指南.md`），目录名使用小写 + 连字符
3. **版本标注**：文档头部通过 YAML Front Matter 注明适用版本与更新时间
4. **配置注释**：所有配置示例必须含中文逐行注释
5. **结构图**：架构图优先使用 Mermaid 语法
